"""Learn-by-doing curriculum: every topic is tied to a concrete pet task."""
...
TRACKS = [
    ...
    ("Object-oriented Python", ["classes", "objects", "__init__", "attributes", "methods", "class variables", "instance variables", "inheritance", "composition", "polymorphism", "properties", "dataclasses", "abstract base classes", "protocols"]),
    ...
    ("Standard library", ["math", "statistics", "random", "datetime", "time", "calendar", "decimal", "fractions", "collections", "itertools", "functools", "heapq", "bisect", "pathlib", "os", "shutil", "glob", "tempfile", "sys", "platform", "subprocess", "argparse", "logging", "inspect", "types", "dis"]),
...
def all_lessons():
    return [(track, topic) for track, topics in TRACKS for topic in topics]

def lesson(track_index=0, topic_index=0):
    track, topics = TRACKS[track_index % len(TRACKS)]
    topic = topics[topic_index % len(topics)]
    return {"track": track, "topic": topic, "goal": f"Use {topic} to change, teach, or care for your pet.", "mode": "Learn -> Do -> See -> Explain -> Experiment -> Challenge"}
