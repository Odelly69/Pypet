...
public final class PetHomeCareView {
    ...
    private static LinearLayout bathroom(Activity a,TextView stats){
        LinearLayout r=room(a,"🚿 BATHROOM","Drag the animated shower head over your pet to wash them. This is a hands-on care action.");
        FrameLayout showerRoom=new FrameLayout(a);showerRoom.setMinimumHeight(430);showerRoom.setBackgroundColor(Color.rgb(220,236,238));
        ...
        final float[] start={0,0};head.setOnTouchListener((v,e)->{if(e.getAction()==MotionEvent.ACTION_DOWN){start[0]=e.getRawX()-v.getX();start[1]=e.getRawY()-v.getY();return true;}if(e.getAction()==MotionEvent.ACTION_MOVE){v.setX(e.getRawX()-start[0]);v.setY(e.getRawY()-start[1]);v.setRotation((float)Math.sin(e.getRawX()/35f)*5f);return true;}if(e.getAction()==MotionEvent.ACTION_UP){float cx=v.getX()+v.getWidth()/2f,cy=v.getY()+v.getHeight()/2f;if(cx>70&&cx<450&&cy>90&&cy<390&&requirePet(a)){head.animate().rotation(0).setDuration(180).start();PetCareSystem.bathe(a);RewardInventory.completeTask(a,"bath",3);refreshStats(a,stats);Toast.makeText(a,"🚿 Shower complete! Your pet is clean.",Toast.LENGTH_SHORT).show();}else{head.animate().x(40).y(20).rotation(0).setDuration(250).start();}return true;}return true;});
        showerRoom.addView(text(a,"Drag 🚿 → over the pet",15,true),new FrameLayout.LayoutParams(-2,60,Gravity.BOTTOM|Gravity.CENTER_HORIZONTAL));r.addView(showerRoom,new LinearLayout.LayoutParams(-1,430));
        return r;
    }
