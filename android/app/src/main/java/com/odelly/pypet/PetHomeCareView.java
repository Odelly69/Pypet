...
public final class PetHomeCareView {
    ...
    private static void addCategory(Activity a,LinearLayout root,String title,String[] ids,String[] labels){TextView h=text(a,title,17,true);h.setPadding(0,10,0,4);root.addView(h);LinearLayout row=new LinearLayout(a);row.setOrientation(LinearLayout.HORIZONTAL);root.addView(row);for(int i=0;i<ids.length;i++){Button food=button(a,labels[i]);final String id=ids[i];food.setOnLongClickListener(v->{if(!requirePet(a))return true;ClipData data=ClipData.newPlainText("food",id);v.startDragAndDrop(data,new View.DragShadowBuilder(v),null,0);return true;});food.setOnClickListener(v->{if(requirePet(a))Toast.makeText(a,"Drag this food to the bowl 🥣",Toast.LENGTH_SHORT).show();});row.addView(food,new LinearLayout.LayoutParams(0,-2,1));}}
