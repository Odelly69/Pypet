...
public final class LivingWorldView {
    ...
    public static void show(Activity a) {
        ...
        root.setBackgroundColor(Color.rgb(128,185,103));
    ...
    private static final class World extends View {
        ...
        @Override protected void onDraw(Canvas c){float fit=Math.min(getWidth()/W,getHeight()/H)*zoom;c.drawColor(Color.rgb(128,185,103));c.save();c.translate(getWidth()/2f+ox,getHeight()/2f+oy);c.scale(fit,fit);drawWorld(c);c.restore();postInvalidateDelayed(300);}
        private void drawWorld(Canvas c){
            p.setStyle(Paint.Style.FILL);p.setColor(Color.rgb(128,185,103));c.drawRect(-W/2,-H/2,W/2,H/2,p);
        ...
        private void drawRoads(Canvas c){
            ...
            // Center markings always run parallel to the road: vertical on vertical roads, horizontal on horizontal roads.
            ...
            // Simple sidewalks make every building lot connect naturally to a street.
        ...
        private void drawSidewalks(Canvas c){
            ...
            // Driveways/paths deliberately terminate at the sidewalk instead of placing buildings on roads.
            ...
            c.drawRoundRect(160,745,250,790,10,10,p);
            c.drawRoundRect(-245,420,-160,465,10,10,p);
        }
        private void drawBuildings(Canvas c){
            house(c,-850,-940,-420,-620,"HOME",0,Color.rgb(164,83,63));
            academy(c,420,-940,850,-620,1);
            market(c,420,290,850,600,2);
            workshop(c,-850,290,-420,600,3);
            library(c,420,760,850,1050,5);
        }
        private void baseBuilding(Canvas c,float l,float top,float r,float bot,int roof,String label,int index){
            ...
            p.setColor(Color.rgb(103,74,54));c.drawRoundRect(m-22,bot-62,m+22,bot,6,6,p);
            t.setTextAlign(Paint.Align.CENTER);t.setTextSize(18);t.setColor(Color.rgb(45,52,47));c.drawText(label,m,top-hh*.31f,t);
        }
        private void window(Canvas c,float x,float y,float w,float h){p.setColor(Color.rgb(87,158,195));c.drawRoundRect(x,y,x+w,y+h,5,5,p);p.setColor(Color.rgb(232,239,227));p.setStrokeWidth(2);c.drawLine(x+w/2,y,x+w/2,y+h,p);c.drawLine(x,y+h/2,x+w,y+h/2,p);}
        ...
        private void workshop(Canvas c,float l,float top,float r,float bot,int index){baseBuilding(c,l,top,r,bot,Color.rgb(48,116,137),"WORKSHOP",index);p.setColor(Color.rgb(122,88,55));c.drawRect(l+50,top+90,r-50,top+125,p);for(int x=0;x<3;x++){p.setColor(Color.rgb(216,171,72));c.drawCircle(l+90+x*125,top+75,16,p);}}
        private void library(Canvas c,float l,float top,float r,float bot,int index){baseBuilding(c,l,top,r,bot,Color.rgb(103,74,137),"LIBRARY",index);for(int x=0;x<4;x++){p.setColor(Color.rgb(101,65,42));c.drawRect(l+45+x*90,top+70,l+70+x*90,bot-70,p);p.setColor(Color.rgb(215,174,84));c.drawRect(l+75+x*90,top+80,l+95+x*90,bot-70,p);}}
        ...
        private void drawPark(Canvas c,float l,float top,float r,float bot,int index){
            ...
            // Paths and lawn sections make the park read as a park at a glance.
            ...
            bench(c,l+125,top+330);bench(c,l+600,top+520);
            drawPond(c,l+90,bot-310);
            drawPlayground(c,r-410,top+150);
            for(int j=0;j<5;j++)tree(c,l+105+j*150,top+90+(j%2)*610,42);
            for(int j=0;j<9;j++)flower(c,l+90+j*88,top+665+(j%2)*30);
        ...
        private void drawPlayground(Canvas c,float x,float y){
            // Slide: platform, ladder and curved-looking chute.
            ...
            // Swings: frame, top bar, chains and seats.
            ...
            // Sandbox and ball make the play area unmistakable.
            p.setColor(Color.rgb(224,190,112));c.drawRoundRect(x+225,y+185,x+365,y+285,18,18,p);p.setColor(Color.rgb(238,214,153));c.drawCircle(x+295,y+235,48,p);
            p.setColor(Color.rgb(224,69,69));c.drawCircle(x+275,y+330,28,p);p.setColor(Color.WHITE);c.drawCircle(x+285,y+320,6,p);c.drawCircle(x+265,y+340,6,p);
            // Seesaw.
            p.setColor(Color.rgb(110,78,52));c.drawRect(x+205,y+360,x+225,y+410,p);p.setColor(Color.rgb(231,143,55));c.drawRoundRect(x+145,y+350,x+285,y+365,8,8,p);
            t.setTextAlign(Paint.Align.CENTER);t.setTextSize(22);t.setColor(Color.rgb(55,83,48));c.drawText("PLAY",x+205,y+445,t);
        }
        ...
        private void drawPet(Canvas c){PetWorldAI.State s=PetWorldAI.tick(a,-1080,1080,-1080,1080);float x=s.x,y=s.y;String species=PetEvolutionManager.current(a).displayName.toLowerCase();if(species.contains("frog"))frog(c,x,y);else generic(c,x,y);t.setTextAlign(Paint.Align.CENTER);t.setTextSize(18);t.setColor(Color.DKGRAY);c.drawText(PetEvolutionManager.name(a)+" • Lv "+PetEvolutionManager.current(a).level,x,y+82,t);}
