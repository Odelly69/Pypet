...
/**
 * Player-facing World: an explorable, connected environment rather than a menu.
 * The rendering is lightweight 3D-style perspective so it works on Android
 * without requiring a heavyweight game engine. Buildings have facades, doors,
 * windows, roofs, paths and interiors. The Academy lab launches real Python
 * missions so learning happens inside the World.
 */
public final class ImmersiveWorldView {
    ...
    public static void show(Activity a) {
        ...
        title.setTextColor(Color.rgb(45,55,48)); title.setPadding(8,10,8,4);
        root.addView(title);
        TextView help = new TextView(a);
        help.setText("Walk anywhere • tap doors to enter • your pet follows • learn by doing");
        ...
        dialog.show();
        if(dialog.getWindow()!=null){ dialog.getWindow().setBackgroundDrawable(new ColorDrawable(Color.TRANSPARENT)); dialog.getWindow().setLayout((int)(a.getResources().getDisplayMetrics().widthPixels*.99f),(int)(a.getResources().getDisplayMetrics().heightPixels*.94f)); }
    }
    ...
    private static final class World3D extends View {
        ...
        private final String[] names={"HOME","ACADEMY","GARDEN","MARKET","WORKSHOP","PARK","LIBRARY"};
        World3D(Activity a){super(a);this.a=a;t.setTypeface(Typeface.create(Typeface.DEFAULT,Typeface.BOLD));setLayerType(View.LAYER_TYPE_SOFTWARE,null);}
        ...
        private void drawHouse(Canvas c,float l,float top,float r,float b,String label,int color,int doorIndex){doors[doorIndex]=new RectF(l+(r-l)*.38f,b-(b-top)*.25f,l+(r-l)*.62f,b-(b-top)*.04f);p.setShadowLayer(9,0,5,Color.argb(70,0,0,0));p.setColor(Color.rgb(245,239,225));c.drawRoundRect(l,top,r,b,10,10,p);p.clearShadowLayer();
            Path roof=new Path();roof.moveTo(l-wid(l,r)*.08f,top);roof.lineTo((l+r)/2,top-(b-top)*.32f);roof.lineTo(r+wid(l,r)*.08f,top);roof.close();p.setColor(color);c.drawPath(roof,p);p.setColor(Color.rgb(250,246,235));c.drawRect(l+5,top+5,r-5,b-5,p);
            ...
            p.setColor(Color.rgb(106,74,52));c.drawRoundRect(doors[doorIndex],6,6,p);p.setColor(Color.rgb(225,185,80));c.drawCircle(doors[doorIndex].right-5,(doors[doorIndex].top+doors[doorIndex].bottom)/2,2,p);
        ...
        private float wid(float l,float r){return r-l;}
        private void drawSchool(Canvas c,float l,float top,float r,float b,String label,int color,int doorIndex){doors[doorIndex]=new RectF((l+r)/2-(r-l)*.07f,b-(b-top)*.27f,(l+r)/2+(r-l)*.07f,b-(b-top)*.03f);p.setShadowLayer(10,0,6,Color.argb(75,0,0,0));p.setColor(Color.rgb(246,241,230));c.drawRoundRect(l,top,r,b,12,12,p);p.clearShadowLayer();Path roof=new Path();roof.moveTo(l,top);roof.lineTo((l+r)/2,top-(b-top)*.38f);roof.lineTo(r,top);roof.close();p.setColor(color);c.drawPath(roof,p);p.setColor(Color.rgb(232,236,235));c.drawRect(l+7,top+7,r-7,b-7,p);
            int cols=4,rows=2;for(int yy=0;yy<rows;yy++)for(int xx=0;xx<cols;xx++){float x=l+(r-l)*(.10f+xx*.25f),y=top+(b-top)*(.24f+yy*.25f);p.setColor(Color.rgb(102,168,198));c.drawRoundRect(x,y,x+(r-l)*.14f,y+(b-top)*.14f,4,4,p);}p.setColor(Color.rgb(91,65,48));c.drawRoundRect(doors[doorIndex],6,6,p);t.setTextAlign(Paint.Align.CENTER);t.setTextSize(Math.max(11,getWidth()*.026f));t.setColor(Color.WHITE);c.drawText("PYTHON LAB",(l+r)/2,top+(b-top)*.65f,t);t.setTextSize(Math.max(10,getWidth()*.023f));t.setColor(Color.rgb(48,56,54));c.drawText(label,(l+r)/2,top-(b-top)*.43f,t);}
        ...
        private void drawPark(Canvas c,float l,float top,float r,float b,String label,int color,int index){p.setColor(color);c.drawRoundRect(l,top,r,b,24,24,p);p.setColor(Color.rgb(126,83,51));c.drawRoundRect(l+(r-l)*.18f,top+(b-top)*.22f,l+(r-l)*.22f,b-(b-top)*.18f,5,5,p);p.setColor(Color.rgb(66,142,74));c.drawCircle(l+(r-l)*.20f,top+(b-top)*.18f,(r-l)*.12f,p);p.setColor(Color.rgb(232,170,70));c.drawCircle((l+r)/2,top+(b-top)*.54f,(r-l)*.12f,p);t.setTextAlign(Paint.Align.CENTER);t.setColor(Color.WHITE);t.setTextSize(Math.max(11,getWidth()*.025f));c.drawText(label,(l+r)/2,b-9,t);doors[index]=new RectF(l,top,r,b);}
        ...
        private void drawHud(Canvas c,float w,float h){p.setColor(Color.argb(225,255,255,255));c.drawRoundRect(w*.025f,h*.025f,w*.72f,h*.105f,18,18,p);t.setTextAlign(Paint.Align.LEFT);t.setColor(Color.rgb(48,57,52));t.setTextSize(Math.max(10,w*.022f));c.drawText("🏠 Home   🏫 Academy   🌳 Garden   🛠 Workshop   🌳 Park",w*.045f,h*.060f,t);c.drawText("Walk with your pet. Doors and places are interactive.",w*.045f,h*.092f,t);}
        @Override public boolean onTouchEvent(MotionEvent e){if(e.getAction()!=MotionEvent.ACTION_UP)return true;float x=e.getX(),y=e.getY();for(int i=0;i<doors.length;i++)if(doors[i]!=null&&doors[i].contains(x,y)){enter(i);return true;}playerX=Math.max(.08f,Math.min(.92f,x/getWidth()));playerY=Math.max(.18f,Math.min(.94f,y/getHeight()));invalidate();return true;}
        ...
        private void home(){new AlertDialog.Builder(a).setTitle("🏠 Your Home").setMessage("A real room-based home for your pet.\n\nBedroom • living room • kitchen • bathroom • study/workshop.\n\nUse care, food, rest and study here. The pet's home grows with balanced development.").setPositiveButton("Study with my pet",(d,w)->codingLab()).setNegativeButton("Back outside",null).show();}
        private void codingLab(){
            ...
            TextView output=new TextView(a);output.setTextSize(15);root.addView(output);
            Button run=new Button(a);run.setText("▶ RUN • TEST • LEARN");root.addView(run);
