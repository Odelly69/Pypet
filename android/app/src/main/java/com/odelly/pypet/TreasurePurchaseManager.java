 ...
 *
 * Production entitlement is fail-closed: the device grants a Treasure only after
 * the configured trusted HTTPS backend verifies the purchase token with Google Play.
 */
public final class TreasurePurchaseManager implements BillingClient.PurchasesUpdatedListener {
        ...
        billing.startConnection(new BillingClientStateListener() {
            ...
            public void onBillingSetupFinished(BillingResult r) {
                if (r.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                    ...
                    message("Google Play Billing unavailable: " + r.getDebugMessage());
                }
            }

            @Override
            public void onBillingServiceDisconnected() {
                message("Google Play Billing disconnected. Try the store again.");
            }
        });
    }
    ...
    private void queryProducts() {
        ...
        for (TreasureCatalog.Item i : TreasureCatalog.all()) {
        ...
        billing.queryProductDetailsAsync(
                QueryProductDetailsParams.newBuilder().setProductList(ids).build(),
                (r, d) -> {
                    if (r.getResponseCode() != BillingClient.BillingResponseCode.OK) {
                        message("Could not load Treasure Trove: " + r.getDebugMessage());
                        return;
                    }
                    products.clear();
                    ...
                    if (listener != null) {
                        listener.onProductsReady(new ArrayList<>(products.values()));
                    }
                    restorePurchases();
                });
    }
    ...
    public void buy(Activity a, String id) {
        ...
        if (d == null) {
            message("That Treasure is not currently available.");
            return;
        }

        BillingFlowParams.ProductDetailsParams p = BillingFlowParams.ProductDetailsParams
                .newBuilder()
                .setProductDetails(d)
                .build();

        BillingResult r = billing.launchBillingFlow(
                a,
        ...
        if (r.getResponseCode() != BillingClient.BillingResponseCode.OK) {
            message("Google Play could not start the purchase: " + r.getDebugMessage());
        }
    }
        ...
        billing.queryPurchasesAsync(
                ...
                (r, ps) -> {
                    if (r.getResponseCode() == BillingClient.BillingResponseCode.OK && ps != null) {
                        for (Purchase p : ps) handlePurchase(p);
                    }
                });
    }
    ...
    public void onPurchasesUpdated(BillingResult r, List<Purchase> ps) {
        if (r.getResponseCode() == BillingClient.BillingResponseCode.OK && ps != null) {
            ...
            message("Purchase was not completed: " + r.getDebugMessage());
        }
    }
    ...
    private void handlePurchase(Purchase p) {
        if (p.getPurchaseState() != Purchase.PurchaseState.PURCHASED) {
            if (p.getPurchaseState() == Purchase.PurchaseState.PENDING) {
                message("Purchase pending. Google Play will notify us when it completes.");
            }
            return;
        }

        for (String id : p.getProducts()) {
            if (isKnownProduct(id)) verifyThenGrant(p, id);
        }
    }

    private boolean isKnownProduct(String id) {
        return products.containsKey(id) || findCatalogItem(id) != null;
    }

    private TreasureCatalog.Item findCatalogItem(String id) {
        for (TreasureCatalog.Item item : TreasureCatalog.all()) {
            if (item.productId.equals(id)) return item;
        }
        return null;
    }

    private void verifyThenGrant(Purchase p, String id) {
        if (verifyUrl == null || verifyUrl.trim().isEmpty()) {
            message("Treasure purchases are temporarily unavailable until purchase verification is configured.");
            return;
        }
        ...
        new Thread(() -> {
            ...
            mainHandler.post(() -> {
                if (!verified) {
                    message("We couldn't verify that purchase yet. Your Treasure has not been granted; please try Restore Purchases later.");
                    return;
                }
    ...
    private boolean verify(Purchase p, String id) throws Exception {
        ...
        body.put("packageName", "com.odelly.pypet");
        body.put("productId", id);
        body.put("purchaseToken", p.getPurchaseToken());
        ...
        try (InputStream in = c.getInputStream();
            ...
            return result.optBoolean("verified", false)
                    && id.equals(result.optString("productId"));
        } finally {
            c.disconnect();
        }
    }
    ...
    private void grant(String id) {
        if (!prefs.getBoolean(id, false)) {
            ...
            if (listener != null) listener.onPurchaseGranted(id);
        }
    }
