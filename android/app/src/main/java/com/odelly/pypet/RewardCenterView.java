...
public final class RewardCenterView {
    ...
    public static void show(Activity a) {
        ...
        Runnable refresh=()->wallet.setText("🪙 Pypet Coins: "+RewardInventory.coins(a)+"   •   🎨 Owned: "+RewardInventory.count(a));refresh.run();
        ...
        for(RewardCatalog.Item item:catalog){if(!item.tier.equals("COIN")&&!item.tier.equals("AD_COIN"))continue;final RewardCatalog.Item x=item;final int slot=index++;Button buy=new Button(a);buy.setAllCaps(false);buy.setText((x.tier.equals("AD_COIN")?"📺 ":"✨ ")+x.name+" • "+x.priceCoins+" coins\n"+x.description);if(RewardInventory.owns(a,x.id))buy.setEnabled(false);buy.setOnClickListener(v->{if(RewardInventory.purchase(a,x.id)){float px=-1000+(slot%8)*250f,py=1120+(slot/8)*125f;WorldPlacementManager.placeUnique(a,x.id,px,py,1f,0f);refresh.run();buy.setEnabled(false);Toast.makeText(a,"Added to your World — go show it off!",Toast.LENGTH_SHORT).show();}else Toast.makeText(a,"Earn more Pypet Coins to unlock this decoration.",Toast.LENGTH_SHORT).show();});items.addView(buy,new LinearLayout.LayoutParams(-1,-2));}
        back.setOnClickListener(v->LivingWorldView.show(a));a.setContentView(root);
    }
}
