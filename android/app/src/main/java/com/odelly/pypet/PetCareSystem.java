...
/** Persistent pet-care layer for hygiene, waste and elapsed-time needs. */
public final class PetCareSystem {
    ...
    public static void ensureRoutine(Context c){
        ...
        if(!hasWaste(c) && (last==0 || now-last>60000L) && RNG.nextInt(100)<45){p(c).edit().putBoolean(WASTE,true).putLong(LAST,now).apply();}
    }
    public static void tick(Context c){
        PetRoutineManager.tick(c);
        ensureRoutine(c);
        int h=hygiene(c);
        if(hasWaste(c)) h=Math.max(0,h-1);
        p(c).edit().putInt(HYGIENE,h).apply();
    }
    public static void cleanWaste(Context c){p(c).edit().putBoolean(WASTE,false).putInt(HYGIENE,Math.min(100,hygiene(c)+15)).apply();}
    public static void bathe(Context c){p(c).edit().putInt(HYGIENE,Math.min(100,hygiene(c)+25)).apply();}
}