...
/**
 * Pypet's core breeding/evolution rules. Every new egg gets an independent random
 * lineage/seed. Evolution quality is driven by a balanced score across all major
 * activities; a separate rarity roll chooses a lineage-specific rare form.
 */
public final class PetEvolutionManager {
    public static final class PetVariant {
        public final String id,species,displayName,emoji,description; public final int level;
        PetVariant(String i,String s,String n,String e,String d,int l){id=i;species=s;displayName=n;emoji=e;description=d;level=l;}
    }
    public static final class PetFood {
        public final String id,name,emoji; public final int hunger,happiness,health;
        PetFood(String i,String n,String e,int h,int a,int l){id=i;name=n;emoji=e;hunger=h;happiness=a;health=l;}
    }
    public static final class Egg {
        public final String id,lineage,emoji,pattern,rarity; Egg(String i,String l,String e,String p,String r){id=i;lineage=l;emoji=e;pattern=p;rarity=r;}
    }
    private static final String PREFS="pypet_pet";
    private static final String CREATED="created", NAME="name", HUNGER="hunger", HAPPY="happiness", HEALTH="health", LESSONS="lessons", PLAY="play", CARE="care", SCHOOL="school", EXPLORE="explore", ROUTINE="routine", LINEAGE="lineage", STAGE="stage", RARITY="rarity", EGG_ID="egg_id", EGG_PATTERN="egg_pattern", EVOLUTIONS="evolutions", EGGS="eggs";
    ...
    private static final String[][] LINEAGES={{"fox","Fox","🦊","ember"},{"unicorn","Unicorn","🦄","moon"},{"dragon","Dragon","🐉","scale"},{"cat","Cat","🐱","whisker"},{"wolf","Wolf","🐺","frost"},{"bunny","Bunny","🐰","flower"},{"turtle","Turtle","🐢","moss"},{"deer","Deer","🦌","leaf"},{"bear","Bear","🐻","honey"},{"frog","Frog","🐸","pond"},{"bird","Bird","🐦","feather"},{"fae","Fae","🧚","spark"},{"spirit","Nature Spirit","🌿","grove"}};
    ...
    private static void init(Context c){if(p(c).getBoolean(CREATED,false))return;Egg e=newEgg();p(c).edit().putBoolean(CREATED,true).putString(NAME,NAMES[RANDOM.nextInt(NAMES.length)]).putInt(HUNGER,75).putInt(HAPPY,75).putInt(HEALTH,90).putString(LINEAGE,e.lineage).putInt(STAGE,0).putString(RARITY,"common").putString(EGG_ID,e.id).putString(EGG_PATTERN,e.pattern).putInt(SCHOOL,0).putInt(EXPLORE,0).putInt(ROUTINE,0).putInt(EVOLUTIONS,0).putInt(EGGS,1).apply();}
    ...
    public static Egg currentEgg(Context c){init(c);return new Egg(p(c).getString(EGG_ID,"egg-1"),p(c).getString(LINEAGE,"fox"),lineageEmoji(p(c).getString(LINEAGE,"fox")),p(c).getString(EGG_PATTERN,"ember"),p(c).getString(RARITY,"common"));}
    ...
    public static Egg hatchNewEgg(Context c){init(c);Egg e=newEgg();String nm=NAMES[RANDOM.nextInt(NAMES.length)];p(c).edit().putString(NAME,nm).putInt(HUNGER,75).putInt(HAPPY,75).putInt(HEALTH,90).putInt(LESSONS,0).putInt(PLAY,0).putInt(CARE,0).putInt(SCHOOL,0).putInt(EXPLORE,0).putInt(ROUTINE,0).putString(LINEAGE,e.lineage).putInt(STAGE,0).putString(RARITY,e.rarity).putString(EGG_ID,e.id).putString(EGG_PATTERN,e.pattern).putInt(EVOLUTIONS,0).putInt(EGGS,eggCount(c)+1).apply();return e;}
    ...
    /** All nine development categories have equal weight. Missing a category cannot be hidden by over-grinding another. */
    ...
    /** Evolves at milestones. Higher balanced care unlocks better tiers; each evolution performs a lineage-specific rare roll. */
    ...
    private static void evolve(Context c,int next){int rareRoll=RANDOM.nextInt(1000);String rarity=rareRoll<5?"mythic":rareRoll<35?"rare":rareRoll<150?"uncommon":"common";p(c).edit().putInt(STAGE,next).putString(RARITY,rarity).putInt(EVOLUTIONS,evolutionCount(c)+1).apply();}
