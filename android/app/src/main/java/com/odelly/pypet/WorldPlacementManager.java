...
public final class WorldPlacementManager {
    ...
    private static final float HALF=1200f,EDGE=70f,ROAD_BLOCK=205f,BUILDING_PAD=60f;
    ...
    private static float[] findSafePosition(Context c,String id,float x,float y,float scale,int ignoreIndex){float radius=footprint(id,scale);x=clamp(x,-HALF+EDGE+radius,HALF-EDGE-radius);y=clamp(y,-HALF+EDGE+radius,HALF-EDGE-radius);if(isValid(c,id,x,y,radius,ignoreIndex))return new float[]{x,y};for(float ring=80;ring<=720;ring+=80){int points=Math.max(8,(int)(ring/35));for(int i=0;i<points;i++){double a=(Math.PI*2*i)/points;float nx=clamp(x+(float)Math.cos(a)*ring,-HALF+EDGE+radius,HALF-EDGE-radius);float ny=clamp(y+(float)Math.sin(a)*ring,-HALF+EDGE+radius,HALF-EDGE-radius);if(isValid(c,id,nx,ny,radius,ignoreIndex))return new float[]{nx,ny};}}return new float[]{id!=null&&id.startsWith("trophy_")?-700f:520f,id!=null&&id.startsWith("trophy_")?-430f:760f};}
    private static boolean isValid(Context c,String id,float x,float y,float radius,int ignoreIndex){if(Math.abs(x)+radius>HALF-EDGE||Math.abs(y)+radius>HALF-EDGE)return false;if(Math.abs(x)<ROAD_BLOCK+radius||Math.abs(y)<ROAD_BLOCK+radius)return false;if(inBuilding(x,y,-850,-940,-420,-620,radius)||inBuilding(x,y,420,-940,850,-620,radius)||inBuilding(x,y,420,290,850,600,radius)||inBuilding(x,y,-850,290,-420,600,radius)||inBuilding(x,y,420,760,850,1050,radius))return false;if(!parkAllowed(id)&&inRect(x,y,-1060,180,-220,1060,radius))return false;return true;}
