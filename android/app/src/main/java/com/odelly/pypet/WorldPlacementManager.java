...
/** Persists player-placed World objects and keeps them inside the actual town map and off roads. */
public final class WorldPlacementManager {
    ...
    // LivingWorldView uses W=2400 and H=3600, so the valid half extents are 1200 and 1800.
    private static final float WORLD_HALF_W=1200f,WORLD_HALF_H=1800f,ROAD_HALF=155f,EDGE=55f;
    ...
    /** Place once by item id; useful for unique display trophies and landmarks. */
    ...
    /** Keep decorations on buildable land, inside the map, and away from the two roads. */
    private static float[] safePosition(float x,float y){x=Math.max(-WORLD_HALF_W+EDGE,Math.min(WORLD_HALF_W-EDGE,x));y=Math.max(-WORLD_HALF_H+EDGE,Math.min(WORLD_HALF_H-EDGE,y));if(Math.abs(x)<ROAD_HALF&&Math.abs(y)<ROAD_HALF){if(Math.abs(x)>=Math.abs(y))x=x>=0?ROAD_HALF+EDGE:-ROAD_HALF-EDGE;else y=y>=0?ROAD_HALF+EDGE:-ROAD_HALF-EDGE;}else if(Math.abs(x)<ROAD_HALF)x=x>=0?ROAD_HALF+EDGE:-ROAD_HALF-EDGE;else if(Math.abs(y)<ROAD_HALF)y=y>=0?ROAD_HALF+EDGE:-ROAD_HALF-EDGE;return new float[]{x,y};}
